...
def test_login_success_creates_auth_token(db_session):
    ...
    assert token
    auth = db_session.scalars(select(Authentication).where(Authentication.token == token)).first()
...
def test_login_wrong_password_does_not_create_token(db_session):
    ...
    assert token is None
    assert db_session.scalar(select(func.count()).select_from(Authentication)) == 0
